%BMED2250 Project - Project Script for Varied Noise
...
agg_mean_accuracy = zeros(1, length(snr_vec));
agg_mean_precision = zeros(1, length(snr_vec));
agg_mean_sensitivity = zeros(1, length(snr_vec));
agg_mean_specificity = zeros(1, length(snr_vec));
...
EMG_tremor_data = awgn(EMG_tremor_data, snr_vec(snr_i));
...
agg_mean_accuracy(snr_i) = mean_accuracy;
agg_mean_precision(snr_i) = mean_precision;
agg_mean_sensitivity(snr_i) = mean_sensitivity;
agg_mean_specificity(snr_i) = mean_specificity;
...
scatter(snr_vec,agg_mean_accuracy)
title('mean accuracy vs. signal-to-noise ratio')
xlabel('signal-to-noise ratio')
ylabel('mean accuracy')
figure
scatter(snr_vec,agg_mean_precision)
title('mean precision vs. signal-to-noise ratio')
xlabel('signal-to-noise ratio')
ylabel('mean precision')
figure
scatter(snr_vec,agg_mean_sensitivity)
title('mean sensitivity vs. signal-to-noise ratio')
xlabel('signal-to-noise ratio')
ylabel('mean sensitivity')
figure
scatter(snr_vec,agg_mean_specificity)
title('mean specificity vs. signal-to-noise ratio')
xlabel('signal-to-noise ratio')
